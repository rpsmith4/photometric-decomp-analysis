...
def main(name, RA, DEC, R, bands='grz', pixscale=0.262, dr='dr9'):
    ...
    for k in range(len(RA)):
        if True:
           ...
           if dr=='dr8':
                url="http://legacysurvey.org/viewer/fits-cutout?ra=%f&dec=%f&width=%i&height=%i&layer=dr8&pixscale=%.3f&bands=%s&subimage" % (RA[k], DEC[k], 2*RR, 2*RR, pixscale,bands)
           else:
                url="http://legacysurvey.org/viewer/fits-cutout?ra=%f&dec=%f&width=%i&height=%i&layer=ls-%s&pixscale=%.3f&bands=%s&subimage" % (RA[k], DEC[k], 2*RR, 2*RR, dr, pixscale,bands)
