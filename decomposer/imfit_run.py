...
def run_imfit(band, 
              mask=True, psf=True, invvar=True, 
              alg="LM", max_threads=4, fit_type="2_sersic", 
              config_file=None, mask_file=None, params_file=None, image_file=None, psf_file=None, invvar_file=None,
              gui_config = None, stdout_callback=None):
    ...
    image_file = image_file or f"image_{band}.fits"
    psf_file = psf_file or f"psf_patched_{band}.fits"
    invvar_file = invvar_file or f"image_{band}_invvar.fits"
    imfit_cmd = "imfit"
    if gui_config:
        if 'imfit_path' in gui_config and gui_config['imfit_path']:
            imfit_cmd = os.path.join(gui_config['imfit_path'], 'imfit')

    command = [imfit_cmd, "-c", config_file, image_file, "--save-params", params_file, "--max-threads", f"{max_threads}"]
    if mask:
        command.extend(["--mask", mask_file or "image_mask.fits"])
    if psf:
        command.extend(["--psf", psf_file])
    if invvar:
        command.extend(["--noise", invvar_file, "--errors-are-weights"])
    if alg=="NM":
