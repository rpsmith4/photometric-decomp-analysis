...
class SersicFitter(BasicFitter):
    name = "Sersic"
    ref_points = ["Central point", "Outer point"]
    n_free_params = 3
    ...
    def get_mu_e(self):
        ...
        n = self.par_values["n"]
        nu = 2 * n - 1/3 + 4 / (405 * n)
        return self.par_values["mu0"] + (2.5 * nu / np.log(10))
    ...
    def evaluate(self, unpack=False):
        ...
        # Sersic approximation
        nu = 2 * n - 1/3 + 4 / (405 * n)

        mag = self.par_values["mu0"] + kappa * nu * (self.radii / self.par_values["re"])**(1/n)

        flux = 10 ** (0.4 * (m0 - mag))
        flux_sym = np.append(flux[::-1], flux)
        flux_conv = fftconvolve(flux_sym, self.psf, mode="same")[len(flux):]

        return -2.5 * np.log10(flux_conv) + m0

    def compute_initials(self, ref_points):
        """
        Use two points to estimate re and mu0.
        """
        (r1, mu1), (r2, mu2) = ref_points

        # Initial guesses
        mu0 = mu1  # central SB
        n = 1.0    # start with exponential-like
        ...
        if dr <= 0 or dmu <= 0:
            re = r2 if r2 > 0 else self.radii[len(self.radii)//3]
        else:
            slope = dmu / dr

            # From Sersic derivative (approx near small radii):
            # dmu/dr ~ (kappa * nu / re^(1/n)) * r^(1/n - 1)
            # assume r ~ re → simplifies to:
            # slope ~ kappa * nu / re
            nu = 2 * n - 1/3 + 4 / (405 * n)
            re = (kappa * nu) / slope

            # keep it in sane range
    ...
    def fit_auto(self):
        """
        Automatic initialization using profile shape
        """
        mu0 = self.surf_bri[0]
        n = 1.0

        # crude slope estimate using inner-to-mid region
        i1 = len(self.radii) // 4
        i2 = len(self.radii) // 2

        r1, r2 = self.radii[i1], self.radii[i2]
        mu1, mu2 = self.surf_bri[i1], self.surf_bri[i2]
    ...
    def chi_penalty(self):
        ...
        if self.par_values["re"] <= 0:
            total *= 100

        if self.par_values["n"] < 0.3:
            total *= 1 + 10 * (0.3 - self.par_values["n"])

        if self.par_values["n"] > 8:
            total *= 1 + (self.par_values["n"] - 8)
